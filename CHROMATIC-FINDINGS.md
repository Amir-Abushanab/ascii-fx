...
>
> This document is the record of _why_ it looks the way it does. The measurement
...
— while reaching for _fewer_ distinct emoji (16.7 vs 23.2 on average). Noto's
...
("it looks like _your_ emoji") and dodges the font-licensing problem, but it is
...
Every result above was palette-limited, so: does _which_ glyphs you bundle
matter more than _how many_? Held-out mean ΔE00 on the Noto pool of 1301, trained
...
that images almost never call for. What matters is _density_, not coverage —
...
## Result 5 — it does not flicker; emoji are _more_ temporally stable than ASCII
...
| _ascii `structural-v1`_ |    _95_ |         _15,833_ |          _12,038_ |           _—_ |
...
_frame_ is about 2× the work. In practice emoji mode wants far fewer columns,
...
_byte-identical_ output for every palette from 50 to 1301 — only 13 glyphs ever
...
- ΔE00's `SC`/`SH` terms discount chroma error at high chroma, so the referee is
  ...
  which would need to _win_ to justify its cost.
