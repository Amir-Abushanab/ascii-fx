...
import { COMPOSITE_WGSL, FEATURES_WGSL, MAX_GPU_GLYPHS, MIPGEN_WGSL, REDUCE_WGSL, matchWgsl } from './shaders.js'
...
export class AsciiEngine {
  ...
  static async create(device: GPUDevice, profile: AsciiProfile): Promise<AsciiEngine> {
    ...
    const [reduce, features, match, mipgen] = await Promise.all([
...
export class AsciiStream {
  ...
  private readonly paramsScratch = new Uint32Array(16)
  ...
  constructor(engine: AsciiEngine, compositePipeline: GPURenderPipeline) {
    ...
    this.paramsBuf = d.createBuffer({ size: 64, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST })
  ...
  configure(src: GPUTexture, srcW: number, srcH: number, opts: StreamMatchOptions): { gridChanged: boolean } {
    ...
    if (opts.temporal && !this.prevReducedBuf) {
  ...
  private syncMatchParams(baseCol = 0, baseRow = 0): void {
    ...
    p[13] = baseCol
    p[14] = baseRow
    p[15] = opts.temporal && this.temporalPrimed && this.prevReducedBuf ? 1 : 0
  ...
  encodeMatchRect(enc: GPUCommandEncoder, baseCol: number, baseRow: number, cols: number, rows: number): void {
    ...
    pass.dispatchWorkgroups(cols, rows)
    pass.end()
    if (this.opts.temporal && this.prevReducedBuf) {
