...
| `interaction`        | Pointer/time effects composited on top (`reveal`, `displace`, `wave`, `push`…). Never re-runs matching. `source: 'motion'` drives them from where the source is moving instead of from the pointer. |
